...
class LocatedVector2D(LocatedVector):
    ...
    def __getVec2VecDistancePointVec(vec1, vec2,
        ...
        for sp in spoints:
            #
            distancePoint = LocatedVector2D._getPoint2VecDistancePoint(vec2,
...
class GrayImageLocatedVector2D(LocatedVector2D):
    ...
    def _getVec2VecDistancePointChargeVec(
        ...
        for sp in spoints:
            #
            distancePoint = LocatedVector2D._getPoint2VecDistancePoint(
                vec2,
                point=sp,
                isMin=isMinFuncs
            )
            ...
            checkval = cls.getConditionDistanceCharge(
    ...
    def setLine(self) -> None:
        ...
        points = []
        for p in plist:
            point = GrayImagePoint(x=p.x, y=p.y, z=0)
            point.setZvalFromImage(self.image)
            points.append(point)
